...
var { userModel, otpModel } = require('../dbrepo/modules')
var router = express.Router();
var SERVER_SECRET = process.env.SECRET || "1234";
// var postmark = require("postmark");
// var emailApi = process.env.EMAIL_API || "c6388475-9e85-4786-8fd1-788ada8c969a";
// var clientApi = new postmark.ServerClient(API_TOKEN);
...
router.post('/forget-password-2', (req, res, next) => {
    ...
    userModel.findOne({ email: req.body.userData.email }, (err, user) => {
        if (err) {
            res.status(500).send({
                message: "Something wrong"
            })
        } else if (user) {
            otpModel.find({ email: req.body.userData.email }, (err, otpData) => {
                if (err) {
                    res.status(500).send({
                        message: "Something wrong"
                    })
